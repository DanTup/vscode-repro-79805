...
["flutter-tester", "chrome"].forEach((deviceId) => {
	describe(`flutter run debugger (launch on ${deviceId})`, () => {
		...
		it("correctly marks debuggable top frames even if not debuggable, if breakpoint/stepping", async function () {
			if (deviceId === "chrome")
				this.skip();
		...
		it("watch expressions provide same info as locals", async function () {
			if (deviceId === "chrome")
				this.skip();
